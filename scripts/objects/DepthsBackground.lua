--- The DepthsBackground object, used in Chapter 1's intro.
---
--- This object does not set its own layer. When creating it, make sure to set its layer appropriately.
---@class DepthsBackground : Object
---@overload fun(...) : DepthsBackground
local DepthsBackground, super = Class(Object)

function DepthsBackground:init(x, y)
    ...
    self.timer:every(40 / 30, function()
        ...
        local piece = self:addChild(DepthsBackgroundPiece(self.sprite, self.width / 2, self.height / 2))
...
function DepthsBackground:update()
    self.music_pitch = MathUtils.approach(self.music_pitch, 1, 0.02 * DTMULT)
    self.music:setPitch(self.music_pitch)

    super.update(self)
end

return DepthsBackground
