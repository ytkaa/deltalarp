return {
    ...
    ofer_intro = function(cutscene, event)
        ...
        cutscene:setSpeaker(tofer)
        cutscene:text("* AND.[wait:5] Check it,[sound:tofer_checkit][wait:5] there is one more thing that makes ME the coolest!")
        cutscene:text("* I have a sweet [color:pink]LIL SIS[color:reset] who plays piano!")
        cutscene:text("* She even wears ribbons and dresses,[wait:5] plays with lil dolls...")
        cutscene:text("* ...and loves [color:pink]PRINCESS-related media[color:reset]!")
        cutscene:text("* Check it[sound:tofer_checkit],[wait:5] SOFER!")
        ...
        cutscene:setSpeaker(tofer)
        cutscene:text("* Check it![sound:tofer_checkit][wait:5] I just couldn't hear 'cause I had my 'pods in!")
        cutscene:setSpeaker(sofer)
        cutscene:text("* Okay.")
        ...
        cutscene:setSpeaker(tofer)
        cutscene:text("[next]* Check it,[sound:tofer_checkit][wait:2] SOFER![wait:2] How about some music?[wait:2] Sofer on the piano,[wait:2] just like")
        Game.world.music:stop()
        cutscene:setSpeaker(sofer)
        ...
        cutscene:wait(2)
        cutscene:setSpeaker(tofer)
        cutscene:text("* I. [wait:10]Don't understand why Sofer's being like this.")
        cutscene:text("* But.")
        cutscene:text("* Heh.[wait:5] Well!")
        cutscene:setAnimation(tofer, "cycle")
        cutscene:wait(cutscene:slideTo(tofer, "tofer_final", 2.5))
        cutscene:look(vess, "right")
        cutscene:text("* Guess it's just me and you then![wait:5] Check it![sound:tofer_checkit]")
    ...
    manor_entrance = function(cutscene, event)
        if Game:hasPartyMember("tofer") then
            ...
            cutscene:setSpeaker(tofer)
            cutscene:text("* No can do,[wait:5] brolio![wait:5] I got three dudes in there doing renovations right now.")
            cutscene:text("* Maybe tomorrow though.[wait:5] Check it![sound:tofer_checkit]")
        else
