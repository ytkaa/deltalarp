return {
    ...
    gardner_chase = function(cutscene, event)
        ...
        cutscene:text("[face:neutral]* Runnin' a farm,[wait:5] y'know?")
        cutscene:text("[face:neutral]* Workin' the land...[wait:5] \nBreathin' fresh air,[wait:5] \nas I said.")
        cutscene:setSprite(gardner, "smile")
        cutscene:text("[face:smile]* Pullin' the weeds![wait:5] \nBuildin' a bond...")
        cutscene:setSprite(gardner, "neutral")
        cutscene:text("[face:neutral]* A life bond![wait:5] Between \nfarmer and crop.")
        music:stop()
        cutscene:wait(1)
        cutscene:walkTo(vess, vess.x - 30, vess.y, 2)
        --cutscene:panTo(camera.x - 30, camera.y, 2)
        cutscene:wait(2)
        cutscene:text("[face:neutral]* ...")
        cutscene:walkTo(vess, vess.x - 30, vess.y, 2)
        --cutscene:panTo(camera.x - 30, camera.y, 2)
        cutscene:wait(2)
        music:play("gardner1")
        cutscene:setSprite(gardner, "smug")
        cutscene:text("[face:smug]* Heheh...[wait:5] the thing about \nrunnin' a farm is...[wait:5] Typically...")
        cutscene:look(vess, "right")
        cutscene:text("[face:smug]* A farmer needs peace.[wait:5] \nAnd quiet.")
        cutscene:setSprite(gardner, "neutral")
        cutscene:text("[face:neutral]* Heheh.[wait:5] See,[wait:5] what I'm \ntryin' to say is...")
        music:fade(0, 2)
        cutscene:wait(3)
        music:stop()
        cutscene:setSprite(gardner, "annoyed")
        cutscene:text("[face:annoyed]* No farmer wants an \ningrate like you \ninterruptin' the Harvest.")
        Assets.playSound("noise")
        cutscene:wait(cutscene:fadeOut(0))
        cutscene:setSprite(gardner, "threat_pink")
        cutscene:slideTo(gardner, "gardner_teleport", 0.1) --Zero-second slide softlocks the game --I shouldve just setPosition here but hey.
