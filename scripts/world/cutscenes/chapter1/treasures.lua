return {
        ...
        if Game:getFlag("debtPaid", 0) >= 1000 then
                ...
                if choice == 1 then
                    ...
                    if choice2 == 1 then
                        ...
                        Game:setFlag("treasureHuntClear", true)
                        local kipkip = cutscene:spawnNPC("kipkip", 1560, 200)
                        cutscene:wait(1.5)
                        cutscene:text("* (Damn)")
                        cutscene:wait(1)
                        cutscene:text("* (Well)")
                        cutscene:wait(1)
                        cutscene:text("* (See ya)")
                        cutscene:wait(cutscene:walkTo(kipkip, "kipkipleave", 0.75))
                        Game.world.music:resume()
                        cutscene:attachFollowers()
                        cutscene:attachCamera()
                    end
    ...
    kipkipsBeatenUp = function(cutscene, event)
        local tofer = cutscene:getCharacter("tofer")
        local vess = cutscene:getCharacter("vess")
        local kipkip = cutscene:spawnNPC("kipkip", 1560, 200)
        cutscene:wait(1.5)
                ...
                local menu = EasingSoulMenuComponent(FitSizing())
                    ...
                    menu:addChild(SoulMenuItemComponent(optionYes, function()
                        ...
                        Game.world:startCutscene(function(cutscene, event) --this somehow works
                            ...
                            Game.inventory:tryGiveItem("treasure_key")
                            Kristal.saveGame()
                            cutscene:wait(cutscene:fadeIn(0.5))
                        end)
                    end))
