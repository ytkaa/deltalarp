---@class FloweryLongLeg1 : Bullet
local FloweryLongLeg1, super = Class(Bullet)
...
function FloweryLongLeg1:init(x, y, dir, speed)
    super.init(self, SCREEN_WIDTH / 2, -200, "bullets/flowerylonglegs/leg1")
    self.timer = Timer()
        self.timer:everyInstant(1.5, function()
            self.target_x = math.random(249, 320)
            self.target_y = 120
            self.start_x = self.x
            self.start_y = self.y
            
            self.arc_timer = 0
            self.arc_duration = 30 -- Duration of the stomp in frames
            self.peak_height = 70  -- How high the arc curves upward
...
function FloweryLongLeg1:onCollide(soul) --stops bullet from disappearing when hitting the soul
    self.destroy_on_hit = false
    super.onCollide(self, soul)
end

function FloweryLongLeg1:update()
    if self.timer then
        self.timer:update()
        if self.moving then
