---@class FloweryLongLeg1crazy : Bullet
local FloweryLongLeg1crazy, super = Class(Bullet)
...
function FloweryLongLeg1crazy:init(x, y, dir, speed)
...
function FloweryLongLeg1crazy:onCollide(soul) --stops bullet from disappearing when hitting the soul
    self.destroy_on_hit = false
    super.onCollide(self, soul)
end

function FloweryLongLeg1crazy:update()
    if self.timer then
        self.timer:update()
        if self.moving then
