local ZobateSpirit, super = Class(Bullet)

function ZobateSpirit:init(x, y, side)
...
function ZobateSpirit:update()
    ...
    if self.delay then
        if self.side == "right" and self.x > SCREEN_WIDTH + self.delay then
            self.wave:spawnBullet("ZobateSpirit", -50, self.random_y, "left")
            self:remove()
        elseif self.side == "left" and self.x < 0 - self.delay then
            self.wave:spawnBullet("ZobateSpirit", 710, self.random_y, "right")
            self:remove()
        end
    end
end
